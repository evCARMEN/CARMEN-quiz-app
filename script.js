...
function buildCategoryButtons(){
  ...
  Object.entries(CATS).forEach(([key, val]) => {
    ...
    b.innerHTML = showLogo ? `<span class="cat-label">${labelText}</span><img src="assets/icons/LandSchafftEnergie.png" alt="LandSchafftEnergie" class="lse-icon">` : `<span class="cat-label">${val.label}</span>`;
...
function handleAnswer(btn,isCorrect,source){
  ...
  if(source) sourceEl.innerHTML=`Quelle: <a href="${source}" target="_blank" rel="noopener">${source}</a>`; else sourceEl.innerHTML='';
...
function nextQuestion(){ idx++; if(idx<currentQuestions.length) showQuestion(); else endQuiz(); }
...
function endQuiz(){
  ...
  currentQuestions.forEach((q,i)=>{
    ...
    breakdown.innerHTML+=`<div class="breakdown-item ${correct?'correct':'wrong'}">Frage ${i+1}: ${correct?'✔️ Richtig':'❌ Falsch'}</div>`;
  });
  const HINTS = [
